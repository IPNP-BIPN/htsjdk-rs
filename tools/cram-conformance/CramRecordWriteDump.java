/*
 * Writing a CRAM record: the same data series, in the same order, going the other way.
 *
 * The read side is pinned. This is the half that produces the bytes, and it is what a port has to
 * get right to write a file at all. Every row carries the record it was given in full, field by
 * field and feature by feature, and the block each series landed in, so a port can rebuild the
 * input and compare the output without either coming from a label.
 *
 ...
 *     record\t<label>\t<index>\t<bam>\t<cram>\t<mate>\t<name>\t<length>\t<ref>\t<start>\t<mateRef>\t<mateStart>\t<template>\t<mq>\t<rg>\t<features>\t<bases>\t<scores>\t<tag list index>
 *     err\t<what>\t<class>\t<message>
 *
 * Usage: CramRecordWriteDump
 */
...
public class CramRecordWriteDump {

    public static void main(final String[] args) {
        System.out.println("# CramRecordWriteDump: the data series a record is read from");
    ...
    static void roundTrip(final String label, final List<CRAMCompressionRecord> records) {
        try {
            ...
            // The slice's own alignment start is the previous one for its first record, which is
            // what the writer is handed. The row carries it so nothing has to be assumed.
            System.out.printf("start\t%s\t%d\t%s%n", label,
                    slice.getAlignmentContext().getAlignmentStart(),
                    slice.getAlignmentContext().getReferenceContext().toString());
            for (int i = 0; i < records.size(); i++) {
                final CRAMCompressionRecord r = records.get(i);
                System.out.printf(
                        "record\t%s\t%d\t%d\t%d\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%s\t%d%n",
                        label, i, r.getBAMFlags(), r.getCRAMFlags(), r.getMateFlags(),
                        String.valueOf(r.getReadName()), r.getReadLength(), r.getReferenceIndex(),
                        r.getAlignmentStart(), r.getMateReferenceIndex(),
                        r.getMateAlignmentStart(), r.getTemplateSize(), r.getMappingQuality(),
                        r.getReadGroupID(), features(r.getReadFeatures()), hex(r.getReadBases()),
                        hex(r.getQualityScores()), r.getTagIdsIndex() == null ? -1
                                : r.getTagIdsIndex().value);
            }
    ...
    /** Each feature as operator, position and payload, so a row rebuilds the record it came from. */
    static String features(final List<ReadFeature> features) {
        ...
        for (final ReadFeature feature : features) {
            ...
            if (feature instanceof InsertBase) {
                payload = hex(new byte[] {((InsertBase) feature).getBase()});
            } else if (feature instanceof Insertion) {
                payload = hex(((Insertion) feature).getSequence());
            } else if (feature instanceof SoftClip) {
                payload = hex(((SoftClip) feature).getSequence());
            } else if (feature instanceof Deletion) {
                payload = Integer.toString(((Deletion) feature).getLength());
            } else {
                payload = "-";
            }
            joiner.add(String.format("%c@%d:%s", feature.getOperator(), feature.getPosition(),
                    payload));
        }
