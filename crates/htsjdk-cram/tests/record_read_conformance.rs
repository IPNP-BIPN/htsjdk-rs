...
fn every_record_reads_back_the_way_the_reference_read_it() {
    ...
    for label in &labels {
            ...
            .filter(|row| row[0] == *label)
        {
            ...
            // The bases an unmapped record carries, and the scores a record keeping them as an
            // array carries. Both come last, and a reader that skips either desynchronises every
            // record after it in the slice.
            assert_eq!(hex(&record.read_bases), row[16], "{label}[{index}] bases");
            assert_eq!(
                hex(&record.quality_scores),
                row[17],
                "{label}[{index}] scores"
            );
            compared += 1;
    ...
    assert_eq!(compared, 13, "records read");
}
...
/// An unmapped record's bases come one at a time out of the same series a mapped record's read
/// feature reads from, so a reader that skips them reads the wrong byte in the record that follows.
#[test]
fn an_unmapped_record_reads_its_bases_from_the_series_the_next_record_uses() {
    let corpus = corpus();
    let slice: Vec<Vec<&str>> = rows(&corpus, "record")
        .into_iter()
        .filter(|row| row[0] == "unmapped-then-mapped")
        .collect();
    assert_eq!(slice.len(), 2);
    assert_eq!(slice[0][16], "43434343434343434343", "ten bases of C");
    assert_eq!(
        slice[1][14], "i@3",
        "and the next record's inserted base is still where it was"
    );

    // And the scores a record keeps as an array are read through the QS series resolved as a byte
    // array rather than as a byte: the same block, read a different way.
    let preserved = rows(&corpus, "record")
        .into_iter()
        .find(|row| row[0] == "scores-preserved")
        .expect("the preserved-scores slice");
    assert_eq!(preserved[3], "3", "detached and quality scores preserved");
    assert_eq!(preserved[17], "0a0b0c0d0e0f10111213");
}
